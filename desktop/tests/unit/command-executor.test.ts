...
describe('CommandExecutor', () => {
  ...
  it('should save and restore clipboard around execution', () => {
    executor.execute([{ type: 'text', value: 'hello' }]);
  ...
  it('should paste text via clipboard + Cmd+V', () => {
    executor.execute([{ type: 'text', value: 'hello world' }]);
  ...
  it('should execute command keystrokes via osascript', () => {
    executor.execute([{ type: 'command', value: 'newline' }]);
    expect(execSync).toHaveBeenCalledWith(
      expect.stringContaining('key code 36')
    );
  });

  it('should skip whitespace-only text segments', () => {
    executor.execute([{ type: 'text', value: '   ' }]);
    // Only clipboard restore should have been written, no paste
    expect(execSync).not.toHaveBeenCalled();
  });

  it('should handle mixed segments in order', () => {
    const segments: ParsedSegment[] = [
      { type: 'text', value: 'hello' },
      { type: 'command', value: 'newline' },
      { type: 'text', value: 'world' },
    ];
    executor.execute(segments);
    expect(execSync).toHaveBeenCalledTimes(3);
  });

  it('should restore clipboard even if execution throws', () => {
    (execSync as unknown as jest.Mock).mockImplementationOnce(() => {
      throw new Error('osascript failed');
    });
    expect(() => {
      executor.execute([{ type: 'text', value: 'test' }]);
    }).toThrow('osascript failed');
    const calls = (clipboard.writeText as jest.Mock).mock.calls;
    expect(calls[calls.length - 1][0]).toBe('original-clipboard');
  });
